...
// grants beyond the literal prefix), and path navigation (`..`, `//`)
// diverges from the pipeline's normalized object keys. The returned prefix
// is trimmed of surrounding slashes; empty is valid (whole bucket).
...
func S3Policy(bucket, prefix string, withDelete bool) (string, error) {
	if bucket == "" || strings.ContainsAny(bucket, "*?/") {
		return "", fmt.Errorf("bucket must be a plain bucket name (no wildcards or slashes)")
	}
